...
class Controller_Admin extends Controller_Template{
	...
	public function action_login(){
		...
		if(isset($post['username'])){
			$check = DB::select('disabled')->from('users')->where('username', '=', $post['username'])->execute()->as_array();
		...
		foreach($user as $value){
			...
			$list .= "
				<li class='AdminUserListItem'>
					<div class='displayInline adminUsersListTitle userName' userId=".$value['id'].">".$value['username']."</div>
					...
					<div class='displayInline adminUsersListTitle'>
						<div class='displayInline manageUser".$accStatus." disableUser adminIcon' style='margin-top:3px;' action='".$accStatus."'></div>
					</div>					
	...
	public function action_manage_users_do(){
		$this->auto_render = false;
		...
		switch($post['action']){
			case 'set_passwrd':
				$password = Auth::instance()->hash_password($post['newPassword']);
				
				DB::update("users")->set(array("password" => $password))->where("id", "=", $post['id'])->execute();
			break;
			
			case 'delete_user':
				DB::delete("users")->where("id", "=", $post['id'])->execute();
			break;
			...
			case 'disable_user':
				if($post['disable_action'] == "enable")
					$action = 0;
				else
					$action = 1;
				
				DB::update("users")->set(array("disabled" => $action))->where("id", "=", $post['id'])->execute();
			break;
	...
	public function action_manage_images(){
		...
		$excludeList = array(".", "..", "user_uploaded");
		...
		if(isset($post['action']) == "changeSection"){
			echo json_encode(array(
				"list" => $list
			));
		}else{
	...
	public function action_upload_category_images(){
		$this->auto_render = false;
		$post = $this->request->post();
		...
		if($validate->check()){
			...
			if($post['name'] == "") $post['name'] = "Image";
			
			//generate unique filename
			$filename = $post['name'] . "_" . str_replace(array(" ", "."), "", microtime()) .".jpg";

			$image = Image::factory($validate['fileName']['tmp_name']);
			
			if($post['section'] == "bg")
				$path = "media/images/designs/backgrounds/".$post['dir']."/";
			else
				$path = "media/images/designs/floating-images/".$post['dir']."/";
			
			//save image
			$image->save($path.$filename);
			
		}else{
			//do multiple error handling
	...
	public function action_about_us(){
		...
		if($post['action'] == "get_content"){
			$content = DB::select()->from("about_us")->execute()->as_array();
			
			isset($content[0]["content"]) ? $html = $content[0]["content"] : $html = "About Us";
			
			echo json_encode(array(
				"html" => $html
			));
		}else{
			DB::update("about_us")->set(array("content" => $post['desc']))->where("id", "=", "1")->execute();
